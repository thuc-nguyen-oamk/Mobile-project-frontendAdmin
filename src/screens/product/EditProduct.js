...
const EditProduct = ({route}) => {
  ...
  useEffect(() => {
    ...
    {
      items.push({label:categoryList[i]['category_name'],value:categoryList[i]['category_id']})
    }
  ...
  const Update = async () => {
    ...
    formData.append('product_id', data[0]);
    ...
    await apis.UpdateProduct(formData, token).then(Alert.alert("Update successful !"));
  };
  return (
